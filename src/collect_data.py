...
Each saved row contains:
    - 126 numeric features: 2 hands * 21 landmarks * 3 coordinates
    - 1 integer label: A-Z => 0-25, 1-9 => 26-34

Controls:
    - Press C to capture the current detected hand pose.
...
def normalize_hand_landmarks(hand_landmarks: object) -> list[float]:
    ...
    Subtracting landmark 0 from every landmark makes each sample more
    position-independent, so the classifier learns hand shape rather than
    where the hand happened to be in the camera frame.
    """
...
def extract_features(results: object) -> tuple[list[float], int]:
    ...
    hand_slots = {"Left": empty_hand.copy(), "Right": empty_hand.copy()}

    hand_landmarks = results.multi_hand_landmarks or []
    handedness_values = results.multi_handedness or []

    for landmarks, handedness in zip(hand_landmarks[:MAX_HANDS], handedness_values[:MAX_HANDS]):
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="Gesture label to collect. Use A-Z or 1-9.",
    )
    parser.add_argument(
        "--camera",
        default=0,
    ...
    parser.add_argument(
        ...
        type=str,
        help="CSV output path. Defaults to data/hand_data.csv.",
    )
...
def draw_status(
    ...
    """Draw concise capture status text onto the webcam frame."""
    status_lines = [
        ...
        with mp_hands.Hands(
            ...
            while True:
                ...
                if key == ord("c"):
                    ...
                    saved_count += 1
                    print(
                        f"Saved sample {saved_count} for label "
                        f"{label_text} ({ID_TO_LABEL[label_id]})."
                    )
    finally:
        camera.release()
